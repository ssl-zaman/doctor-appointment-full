...
const Index = () => {
    ...
    function submitMedicine(e) {
        ...
        if (data.id) {
            setAddMode(true)
            put(route('dosetime.update', data.id), {
                // method: 'PUT',
                data: data,
            });
        } else {
            post(route('dosetime.store'), {
                data: data,
            });
        }
    ...
    function handleDelete(id) {
        if (window.confirm("Are you sure you want to delete this medicine?")) {
            ...
            deleteMedicine(route('dosetime.destroy', id));
        }
    }
    ...
    <MainLayout>
        ...
        <div className="flex gap-10">
            ...
            <div className="w-full bg-white p-5">
                <h2 className="text-2xl font-bold mb-5">Dosetime Manages</h2>
                ...
                <table className="w-full">
                    ...
                    <tbody>
                            {dosetime?.map((item, index) => (
                                <tr key={index} className="border-b border-gray-300">
                                    <td>{index + 1}</td>
                                    <td>{item.name}</td>

                                    <td className="flex gap-1">
                                        <button onClick={() => editMedicine(item.id, item.name, )} className="text-blue-500">
